#!/usr/bin/env python3
"""
Export unprocessed Gmail messages to YAML format.
"""
...
def get_unprocessed_emails(service, max_results=None):
    """Get all unread emails using pagination."""
    try:
        ...
        print(f"\n✅ Found {len(all_messages)} total unread messages")
...
def main():
    parser = argparse.ArgumentParser(
        description="Export unprocessed Gmail messages to YAML"
    )
    parser.add_argument(
        "--output-dir",
    ...
    email_data = get_unprocessed_emails(service, max_results=args.max_results)

    if email_data:
        print("💾 Saving to YAML file...")
        save_to_yaml(email_data, output_dir)
    else:
        print("✨ No unread emails to export!")
